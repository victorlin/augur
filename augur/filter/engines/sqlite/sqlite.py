...
from augur.io.db.sqlite import (
    TabularFileLoaderSQLite,
    cleanup,
    ROW_ORDER_COLUMN,
    sanitize_identifier,
...
from .db_base import (
    DUMMY_COL,
    FilterDbBase,
    FilterCallableReturn,
    FilterOption,
)
from .subsample import get_sizes_per_group
...
# A special value for FILTER_REASON_COL added by subsampling
SUBSAMPLE_FILTER_REASON = 'subsampling'


class FilterSQLite(FilterDbBase):
    def __init__(self, args:argparse.Namespace, db_file:str='', in_memory_db:bool=False):
        super().__init__(args)
        self.using_in_memory_db = in_memory_db
        if self.using_in_memory_db:
            # Use a singleton connection to an in-memory database.
    ...
    def get_db_context(self, **connect_kwargs) -> sqlite3.Connection:
        ...
        # TODO: use isolation_level=None if all commands are predictably
        # sequential so transactions aren't needed?
        # https://docs.python.org/3/library/sqlite3.html#sqlite3.Connection.isolation_level
        return sqlite3.connect(self.db_file, **connect_kwargs)

    def db_set_sanitized_identifiers(self):
        """Sets sanitized names for externally sourced identifiers."""
        # Note that sanitized identifiers should be unchanged in the final output.
    ...
    def db_create_strain_index(self, table_name:str):
        """Creates a unique index on `self.sanitized_metadata_id_column` in a table."""
    ...
    def db_create_date_table(self):
        ...
        with self.get_db_context() as con:
            if self.has_date_col:
                ...
                # Create a placeholder table for later JOINs
                con.execute(f"""CREATE TABLE {DATE_TABLE_NAME} AS
                    SELECT
                        {self.sanitized_metadata_id_column},
                        '' AS {DATE_YEAR_COL},
    ...
    def db_update_filter_reason_table_with_subsampling(self, group_by_cols:List[str]):
        ...
        # Construct a SQL query by parts.

        # First, create a list of conditions to:

        # 1. Drop strains with a null group-by value.
        where_conditions = []
        sanitized_group_by_cols = [sanitize_identifier(col) for col in group_by_cols]
        for col in sanitized_group_by_cols:
            where_conditions.append(f'{col} IS NOT NULL')

        # 2. Factor in the group size limit.
        # group_i is defined in the big query below. It represents an incremental number
        # ordered by priority within each group, where the highest priority starts at 0.
        where_conditions.append(f'group_i <= {GROUP_SIZE_COL}')

        # Create a query to subsample from the pre-filtered table.
        # To treat rows without priorities as lowest priority,
        # `ORDER BY … NULLS LAST` would be ideal. However, that syntax is
        # unsupported for SQLite <3.30.0 so `CASE … IS NULL` is equivalent.
        # https://stackoverflow.com/a/12503284
        ...
        # Apply subsampling to the final outputs by updating the filter reason
        # table with subsampling as a reason.
        with self.get_db_context() as con:
            con.execute(f"""
                UPDATE {METADATA_FILTER_REASON_TABLE_NAME}
                SET
    ...
    def db_generate_priorities_table(self, seed:int=None):
        ...
        # Random seeding is not possible with SQLite [1], so use a workaround:
        # 1. Export the strain IDs into a pandas DataFrame
        # 2. Create a new column with random priority values
        # 3. Import the DataFrame as a new table in the database
        # [1]: https://stackoverflow.com/a/24394275
    ...
    def db_create_group_sizes_table(self, group_by_cols:List[str], sequences_per_group:float):
        ...
        # Extend the above DataFrame with a new column that indicates the number of strains to include in each group.
